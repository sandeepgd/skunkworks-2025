...
## Get user
```bash
curl "http://localhost:3000/users?userId=U681e51661c7154e79471e280"
OR
curl "http://localhost:3000/users?phoneNumber=%2B18324217365"
```
...
```bash
curl -X POST \
  http://localhost:3000/convertTts \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST \
  http://localhost:3000/convertStt \
